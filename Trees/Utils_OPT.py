...
def plt_tree_to_plt_Tree(plt_tree,f_old):
    ...
    plt_Tree=np.zeros((len(plt_tree),3),dtype=int)
    f=np.zeros((len(plt_tree),))
    
    for i,pt in enumerate(plt_tree):
        plt_Tree[i,:]=[i,int(pt[1])-1,int(pt[2])-1]
        f[i]=f_old[pt[0]]
...
def plt_Tree_to_plt_tree(plt_Tree,f_old):
    ...
    for i,pt in enumerate(plt_Tree):
        plt_tree.append([int(i),str(pt[1]+1),str(pt[2]+1),'inf'])
        f[i]=f_old[pt[0]]
...
#     print(f.shape, f_uniq.shape)
    ...
    plt_tree = np.zeros((Z.shape[0]+n_pts,3), dtype = int)
